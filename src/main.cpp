...
void initMotor(BLDCMotor* motor, Encoder* encoder, BLDCDriver3PWM* driver) {
 ...
 // Handle and initialize motor
  ...
  motor->PID_velocity.I = 20; //for correcting velocity error
  motor->PID_velocity.D = 0.001; //damps sudden changes to angle velocity
  // jerk control using voltage voltage ramp
  // default value is 300 volts per sec  ~ 0.3V per millisecond
  motor->PID_velocity.output_ramp = 1000;

  motor->P_angle.P = 20;   // angle P controller -  default P=20
  // velocity low pass filtering
  // default 5ms - try different values to see what is the best. 
  // the lower the less filtered
  motor->LPF_velocity.Tf = 0.01;
  // set to target velocity limit
  motor->velocity_limit = 10.0; //maximal velocity of the position control
  //key for reducing torque!
  motor->voltage_limit = 11.0;
...
void setup() {
  ...
  simulation.initSingle(&encoder_0, &driver_0, &motor_0);
...
void loop() {
  ...
  if (simulation.current != simulation.previous) {
    Serial.println(SimulationStateToString(simulation.current));
    simulation.newState();
  }
